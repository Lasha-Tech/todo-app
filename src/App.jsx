...
function App() {
  ...
  //Filter Colors
  const [clearColor, setClearColor] = useState('#9495A5')
  ...
  const handleDayNightClick = () => {
    ...
    setFilter('all')
    if (!day) {
      setClearColor('#9495A5')
      setAllColor('#3A7CFD')
      setActiveColor('#9495A5')
      setCompleteColor('#9495A5')
    } else {
      setClearColor('#5B5E7E')
      setAllColor('#3A7CFD')
      setActiveColor('#5B5E7E')
      setCompleteColor('#5B5E7E')
    }
  ...
  const handleActiveMouseOver = () => {
    if (activeColor == '#3A7CFD') {
      return
    } else {
      if (day) {
        setActiveColor('#494C6B')
      } else {
        setActiveColor('#E3E4F1')
      }
    }
  }
  const handleCompleteMouseOver = () => {
    if (completeColor == '#3A7CFD') {
      return
    } else {
      if (day) {
        setCompleteColor('#494C6B')
      } else {
        setCompleteColor('#E3E4F1')
      }
    }
  }
  const handleClearMouseOver = () => {
    if (day) {
      setClearColor('#494C6B')
    } else {
      setClearColor('#E3E4F1')
    }
  }
  //Mouse Out Footer Colors
  const handleAllMouseOut = () => {
    if (allColor == '#3A7CFD') {
      return
    } else {
      if (day) {
  ...
  return (
    <div className='App' style={{ backgroundColor: day ? '#FFF' : '#171823' }}>
      <div className="header">
        ...
        <div className="backgroundImgColor" style={{
          background: day ? 'linear-gradient(to left, #5596FF, #AC2DEB)'
      ...
      <div className="wrapper">
        ...
        <div className="input-container" style={{
          backgroundColor: day ? '#FFF' : '#25273D', boxShadow: day ? '0px 35px 50px -15px rgba(194, 195, 214, 0.50)'
            : '0px 35px 50px -15px rgba(0, 0, 0, 0.50)'
        }}>
          <div className="check" onClick={() => setChecked(!checked)} style={{ border: day ? '1px solid #E3E4F1' : '1px solid #393A4B' }}>
          ...
          <form className='form' onSubmit={e => handleSubmitFunc(e)}>
            <input type="text" style={{ backgroundColor: day ? '#FFF' : '#25273D', color: day ? '#393A4B' : '#C8CBE7' }}
            ...
            return (
              <div key={index} className="todo-item"
                ...
                <div className="left">
                  ...
                  {/* Text & Checkbox test*/}
                  <div className="check-todo" onMouseOver={(e) => e.target.style.borderColor = '#8dbdbd'}
                    onMouseOut={(e) => e.target.style.borderColor = day ? '#E3E4F1' : '#393A4B'}
                    onClick={() => handleTodoItemClick(index)} style={{ borderColor: day ? '#E3E4F1' : '#393A4B' }}>
